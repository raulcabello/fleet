...
const numberOfFilesInHelmTestRepo = 9
	...
	When("apply a folder with fleet.yaml that contains a helm release", func() {
		When("no auth is required", func() {
			...
			AfterAll(func() {
				...
				err = env.Fclient.Fleet.Bundle().Delete(testenv.Namespace, "assets-helm", &metav1.DeleteOptions{})
				Expect(err).NotTo(HaveOccurred())
			})
			It("fleet apply success", func() {
				err := fleetApply("helm", []string{testenv.AssetsPath + "helm"}, &apply.Options{})
				Expect(err).NotTo(HaveOccurred())
			})
		...
		When("auth is required, and it is provided without --helm-repo-url-regex", func() {
			...
			It("fleet apply success", func() {
				err := fleetApply("helm", []string{testenv.AssetsPath + "helm"}, &apply.Options{Auth: bundlereader.Auth{Username: username, Password: password}})
				Expect(err).NotTo(HaveOccurred())
			})
...
func verifyResourcesArePresent() bool {
	bundle, err := env.Fclient.Fleet.Bundle().Get(testenv.Namespace, "assets-helm", metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred())
